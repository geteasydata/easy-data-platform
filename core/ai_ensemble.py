...
class AIEnsemble:
    ...
    # Default API keys (Will be loaded from st.secrets)
    ...
    def __init__(self):
        ...
        try:
            self.groq_key = st.secrets["api_keys"]["GROQ_API_KEY"]
        except:
            self.groq_key = None
            
        try:
            self.deepseek_key = st.secrets["api_keys"]["DEEPSEEK_API_KEY"]
        except:
            self.deepseek_key = None
            
        try:
            self.gemini_key = st.secrets["api_keys"]["GEMINI_API_KEY"]
        except:
        ...
        if HAS_GROQ and self.groq_key:
            try:
             ...
             self.log("⚠️ Groq Key missing in secrets")
        
        # Setup DeepSeek
        if HAS_REQUESTS and self.deepseek_key:
        ...
        elif not self.deepseek_key:
             self.log("⚠️ DeepSeek Key missing in secrets")
        ...
        if HAS_GEMINI and self.gemini_key:
            try:
             ...
             self.log("⚠️ Gemini Key missing in secrets")
